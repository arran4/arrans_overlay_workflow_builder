...
SRC_URI="
  amd64? ( https://github.com/rustdesk/rustdesk/releases/download/1.2.6/rustdesk-${PV}-x86_64.AppImage -> ${P}-rustdesk-${PV}-x86_64.AppImage )
"

src_unpack() {
  if use amd64; then
    cp "${DISTDIR}/${P}-rustdesk-${PV}-x86_64.AppImage" "rustdesk.AppImage"  || die "Can't copy downloaded file"
  fi
  ...
  ./rustdesk.AppImage --appimage-extract "usr/share/icons" || die "Failed to extract icons from app image"
}
