# Feature Request: Add pipeline data extraction capability (`g2 extract` or `g2 pipeline`)
...
A new command (e.g., `g2 extract` or a subcommand under `g2 pipeline`) is needed to safely parse and execute a declarative extraction expression. The workflow-builder would emit a human-readable command like:
...
*   **Zero results**: Must result in an actionable failure (non-zero exit code).
...
*   **Legitimate falsey scalar values** (like `"0"` or `""` if they match the extraction logic) must be preserved and output correctly without failing.
