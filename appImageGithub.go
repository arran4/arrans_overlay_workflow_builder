...
// (including the "Argument Injection" and "Denial of Service" vulnerabilities). This is a transitive dependency that
// cannot be easily updated because the upstream library depends on this older version. The risk of this vulnerability
// is accepted for now, as the application is not using the git functionality of `go-appimage` in a way that is exposed
// to the vulnerability. The vulnerability is related to maliciously crafted Git server replies, and this application
// does not interact with git servers through the `go-appimage` library.
import (
	"archive/tar"
	"archive/zip"
	"compress/bzip2"
