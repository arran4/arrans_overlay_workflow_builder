...
func TestPythonPipeline(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/rss":
			...
			w.Write([]byte(`<rss><channel><item><link>http://example.com/v1.0.tar.gz</link></item></channel></rss>`))
		case "/atom":
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`<feed xmlns="http://www.w3.org/2005/Atom"><entry><link href="http://example.com/v2.0.tar.gz"/></entry></feed>`))
		case "/json":
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"releases": [{"tag": "v2.0"}]}`))
		case "/html":
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`<html><body><a href="relative.tar.gz">link</a><a href="other.tar.gz">link2</a></body></html>`))
		case "/xml":
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`<root><link>http://example.com/v3.0.tar.gz</link></root>`))
		default:
